...
def _build_model(constants, teacher_forcing_prob):
    encoder_inputs = keras.Input(shape=(
        constants['MAX_CHORALE_LENGTH'], constants['X_DIM']), name='chorale_input')
    chorale_masking_layer = keras.layers.Masking(
        mask_value=constants['MASK_VALUE'], name='chorale_masking')
    masked_encoder_inputs = chorale_masking_layer(encoder_inputs)

    encoder_1 = keras.layers.LSTM(
        LATENT_DIM, return_sequences=True, return_state=True, name='encoder_lstm_1')
    encoder_2 = keras.layers.LSTM(
        LATENT_DIM, return_sequences=True, return_state=True, name='encoder_lstm_2')
    encoder_3 = keras.layers.LSTM(
        LATENT_DIM, return_sequences=True, return_state=True, name='encoder_lstm_3')

    encoder_outputs_1, _, _ = encoder_1(masked_encoder_inputs)
    encoder_outputs_2, _, _ = encoder_2(encoder_outputs_1)
    encoder_outputs, encoder_state_h, encoder_state_c = encoder_3(
        encoder_outputs_2)

    decoder_inputs = keras.Input(
        shape=(constants['MAX_ANALYSIS_LENGTH'], constants['Y_DIM']), name='rna_input')
    ...
    decoder_layer = DecoderLayer(
        units=LATENT_DIM, constants=constants, teacher_forcing_prob=teacher_forcing_prob)
    decoder_recurrent = keras.layers.RNN(
        decoder_layer, return_sequences=True, return_state=True, name='decoder_layer')
    ...
    initial_attn_energies = keras.layers.Lambda(
        lambda x: x - x - 5)(tf.reduce_sum(encoder_outputs, axis=-1))
    initial_lstm_states = [
        encoder_state_h,
        encoder_state_h,
        encoder_state_h,
    ...
    initial_decoder_state = [
        initial_dense_ins, initial_lstm_states, initial_attn_energies, encoder_outputs]
    dense_outputs, attn_energies, _, lstm_out_states, _, _ = decoder_recurrent(
        masked_decoder_inputs, initial_state=initial_decoder_state)
...
def train(teacher_forcing_prob):
    ...
    model_checkpoint_callback = keras.callbacks.ModelCheckpoint(
        filepath='thrush_attn_unidir_istermmse_lr001B08_luongattn/{epoch:02d}',
    ...
    model.save(f'thrush_attn_unidir')
...
def predict(epochs, teacher_forcing_prob):
    ...
    model.load_weights(f'thrush_attn_unidir/variables/variables')
    ...
    dense_out_inputs = keras.Input(
        shape=(constants['Y_DIM']), name='dense_out_inputs_inference')
    ...
    encoder_output_input = keras.Input(shape=(
        constants['MAX_CHORALE_LENGTH'], LATENT_DIM), name="encoder_output_input")

    decoder_recurrent = _get_layers('recurrent.RNN')[0]

    decoder_states_inputs = [dense_out_inputs, decoder_state_inputs,
    ...
    def decode(input_seq, decoder_input=None):
        (prev_decoder_state_h, prev_decoder_state_c), encoder_output_values = encoder_model.predict(
            np.array([input_seq]))
        prev_decoder_states = [
...
if __name__ == '__main__':
    if sys.argv[1] == "train":
        ...
        np.save(f'attn_unidir_{str(sys.argv[4])}.npy', mats)
